...
func TestMCPServerTableStatsExecution(t *testing.T) {
	...
	// Verify result - can be []TableStats or []interface{}
	if resp.Result == nil {
	...
	case []handlers.TableStats:
		if len(v) == 0 {
			t.Fatal("table_stats returned empty result")
		}
	case []interface{}:
		if len(v) == 0 {
			t.Fatal("table_stats returned empty result")
...
// TestMCPServerQueryAnalysisExecution tests end-to-end query_analysis execution
func TestMCPServerQueryAnalysisExecution(t *testing.T) {
	reader := bytes.NewReader([]byte{})
	writer := &bytes.Buffer{}
	...
	resp := mcp.HandleRequest(req)
	if resp.Error != nil {
		t.Fatalf("query_analysis execution failed: %v", resp.Error)
	}

	// Verify response is a map or QueryAnalysisResult (single result, not array)
	...
	if _, ok := result["query_id"]; !ok {
		t.Fatal("Missing query_id in result")
	}

	if _, ok := result["query_text"]; !ok {
		t.Fatal("Missing query_text in result")
	}

	if _, ok := result["execution_count"]; !ok {
		t.Fatal("Missing execution_count in result")
	}

	if _, ok := result["anomalies"]; !ok {
		t.Fatal("Missing anomalies in result")
	}

	if _, ok := result["recommendations"]; !ok {
		t.Fatal("Missing recommendations in result")
	}
}

// TestMCPServerIndexSuggestExecution tests end-to-end index_suggest execution
func TestMCPServerIndexSuggestExecution(t *testing.T) {
	reader := bytes.NewReader([]byte{})
	writer := &bytes.Buffer{}
	...
	resp := mcp.HandleRequest(req)
	if resp.Error != nil {
		t.Fatalf("index_suggest execution failed: %v", resp.Error)
	}

	// Verify result - can be []IndexSuggestion or []interface{}
	if resp.Result == nil {
		t.Fatal("index_suggest returned nil result")
	}
...
// TestMCPServerAnomalyDetectExecution tests end-to-end anomaly_detect execution
func TestMCPServerAnomalyDetectExecution(t *testing.T) {
	reader := bytes.NewReader([]byte{})
	writer := &bytes.Buffer{}
	...
	resp := mcp.HandleRequest(req)
	if resp.Error != nil {
		t.Fatalf("anomaly_detect execution failed: %v", resp.Error)
	}

	// Result can be empty list if no anomalies
	if resp.Result == nil {
		t.Fatal("anomaly_detect returned nil result")
	}
	...
	case []interface{}:
		if len(v) > 0 {
			...
			if _, ok := alert["metric_name"]; !ok {
				t.Fatal("Missing metric_name in alert")
			}

			if _, ok := alert["severity"]; !ok {
				t.Fatal("Missing severity in alert")
			}
		}
	default:
		t.Fatalf("Unexpected result type: %T", resp.Result)
...
func TestMCPServerConcurrentRequests(t *testing.T) {
	...
	requests := []server.JSONRPCRequest{
		{
			JSONRPC: "2.0",
			ID:      1,
			Method:  "tools/call",
...
// TestMCPServerRequestResponseCycle tests full request-response cycle
func TestMCPServerRequestResponseCycle(t *testing.T) {
	// Create a pipe for stdio simulation
	input := bytes.NewBufferString("")
	output := &bytes.Buffer{}

	tr := transport.NewStdioTransport(input, output)
	mcp := server.NewMCPServer(tr)
	...
	// Verify response
	if resp.JSONRPC != "2.0" {
		t.Errorf("Invalid JSONRPC: %s", resp.JSONRPC)
	}

	if resp.ID != 1 {
		t.Errorf("Invalid ID: %d", resp.ID)
	}
	...
	// Marshal response to JSON to simulate actual response
	...
	if len(respBytes) == 0 {
		t.Fatal("Response bytes are empty")
	}
}
...
// TestMCPServerCompleteWorkflow tests a complete realistic workflow
func TestMCPServerCompleteWorkflow(t *testing.T) {
	reader := bytes.NewReader([]byte{})
	writer := &bytes.Buffer{}
	...
	// 5. Get index suggestions
	// 6. Detect anomalies

	workflow := []struct {
		name   string
		req    server.JSONRPCRequest
		verify func(*testing.T, server.JSONRPCResponse)
	}{
		{
			name: "Initialize",
			req: server.JSONRPCRequest{
			...
			verify: func(t *testing.T, resp server.JSONRPCResponse) {
				if resp.Error != nil {
		...
		},
		{
			...
			verify: func(t *testing.T, resp server.JSONRPCResponse) {
				if resp.Error != nil {
					t.Errorf("List tools error: %v", resp.Error)
				}
			},
		},
		{
			...
			verify: func(t *testing.T, resp server.JSONRPCResponse) {
				if resp.Error != nil {
					t.Errorf("Table stats error: %v", resp.Error)
				}
			},
		},
		{
			...
			req: server.JSONRPCRequest{
				...
				Params: map[string]interface{}{
					"name":   "query_analysis",
					"params": map[string]interface{}{"query_id": "q_123"},
				},
			},
			verify: func(t *testing.T, resp server.JSONRPCResponse) {
				if resp.Error != nil {
					t.Errorf("Query analysis error: %v", resp.Error)
				}
			},
		},
		{
			...
			verify: func(t *testing.T, resp server.JSONRPCResponse) {
				if resp.Error != nil {
					t.Errorf("Index suggest error: %v", resp.Error)
				}
			},
		},
		{
			...
			verify: func(t *testing.T, resp server.JSONRPCResponse) {
				if resp.Error != nil {
					t.Errorf("Anomaly detect error: %v", resp.Error)
				}
			},
		},
	}
	...
	for _, step := range workflow {
		resp := mcp.HandleRequest(step.req)
		step.verify(t, resp)
	}
}
